...
<?php
$search_sql2 = "SELECT * FROM parcel WHERE status!='Completed' AND office='Houston'";
  $res_2 = mysqli_query($con,$search_sql2);
  if($res_2){
    while($row = mysqli_fetch_assoc($res_2)){
      $id=$row['packageID'];
      $weight=$row['weight'];
      if($weight > 150){
        ?> <h5 style="color:red"> Attention! Heavy Item ID: <?php echo $id ?> Send to hanlding </h5> <?php
      }
    }
  }
?>
...
<?php
  $search_sql2 = "SELECT * FROM parcel WHERE status!='Completed' AND office='San Antonio'";
    $res_2 = mysqli_query($con,$search_sql2);
    if($res_2){
      while($row = mysqli_fetch_assoc($res_2)){
        $id=$row['packageID'];
...
<?php
  $search_sql2 = "SELECT * FROM parcel WHERE status!='Completed' AND office='El Paso'";
  $res_2 = mysqli_query($con,$search_sql2);
  if($res_2){
    while($row = mysqli_fetch_assoc($res_2)){
      $id=$row['packageID'];
